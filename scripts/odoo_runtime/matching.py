...
def _score(source: ContactIdentity, candidate: ContactCandidate) -> tuple[int, tuple[str, ...]]:
    score = 0
    reasons: list[str] = []
    if normalize_email(source.email) and normalize_email(source.email) == normalize_email(candidate.email):
        score += 100
        reasons.append("exact_email")
    source_phone = normalize_phone(source.phone)
    if source_phone and source_phone in {normalize_phone(candidate.phone), normalize_phone(candidate.mobile)}:
        score += 80
