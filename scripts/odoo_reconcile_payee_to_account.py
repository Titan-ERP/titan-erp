...
PLAN = OUT / "odoo_payee_account_reconcile_plan.csv"

TARGET_COMPANY_NAME = "Southern Equipment Company (Laurel)"
TARGET_JOURNAL_NAME = "Bank"
PAYEE_PATTERN = r"Joshua McLain"
TARGET_ACCOUNT_NAME = "Service Cost of Revenue"
...
def single_id(models, db, uid, api_key, model, domain, label):
    rows = read(models, db, uid, api_key, model, domain, ["id", "name"], limit=2)
    if len(rows) != 1:
        raise SystemExit(f"Expected one {label}; found {len(rows)}")
    return rows[0]["id"]


def account_id(models, db, uid, api_key, company_id):
    ...
    rows = read(models, db, uid, api_key, "account.account", domain, ["id", "name", "code"], limit=2)
    if len(rows) != 1:
        raise SystemExit(f"Expected one account named {TARGET_ACCOUNT_NAME!r} for company id {company_id}; found {len(rows)}")
    return rows[0]
...
def main():
    ...
    company_id = single_id(models, db, uid, api_key, "res.company", [("name", "=", TARGET_COMPANY_NAME)], TARGET_COMPANY_NAME)
    journal_id = single_id(
        models,
        db,
        uid,
        api_key,
    ...
    account = account_id(models, db, uid, api_key, company_id)
    ...
    bank_lines = read(
        ...
        [
            ...
            ("is_reconciled", "=", False),
            ("payment_ref", "ilike", PAYEE_PATTERN),
        ],
    ...
    for bank in bank_lines:
        if not re.search(PAYEE_PATTERN, bank.get("payment_ref") or "", re.I):
            continue
        suspense_lines = [
        ...
        plan_rows.append(
            {
                ...
                "Old Account": rel_name(suspense.get("account_id")),
    ...
    with PLAN.open("w", newline="", encoding="utf-8-sig") as f:
        fields = ["Bank Statement Line ID", "Date", "Payment Ref", "Amount", "Suspense Move Line ID", "Old Account", "New Account ID", "New Account"]
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        writer.writerows(plan_rows)

    if apply:
        for line_id, new_account_id in writes:
            execute(models, db, uid, api_key, "account.move.line", "write", [[line_id], {"account_id": new_account_id}])

    print(f"Connected uid: {uid}")
    print(f"Payee pattern: {PAYEE_PATTERN}")
