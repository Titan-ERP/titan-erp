...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("Usage: odoo_install_module.py MODULE_NAME")
    ...
    if module["state"] == "installed":
        print("Already installed; no action taken.")
        return
    if module["state"] not in {"uninstalled", "to install"}:
        raise SystemExit(f"Module is in state {module['state']!r}; not installing automatically.")

    execute(models, db, uid, api_key, "ir.module.module", "button_immediate_install", [[module["id"]]])
    ...
    if module["state"] != "installed":
        raise SystemExit(f"Install did not reach installed state; current state: {module['state']}")
