...
class ProductTemplate(models.Model):
    ...
    def _southern_website_quick_facts(self):
        ...
        if product.southern_oem_reference_ids:
            facts.append({"label": "OEM Cross References", "value": len(product.southern_oem_reference_ids)})
        if product.southern_fitment_ids:
            make_count = len(product.southern_fitment_ids.mapped("make_id"))
            model_count = len(product.southern_fitment_ids.mapped("model_id"))
            facts.append({"label": "Fits", "value": f"{make_count} makes / {model_count} models"})
        if product.southern_catalog_page_ids:
            facts.append({"label": "Catalog References", "value": len(product.southern_catalog_page_ids)})
        return facts
