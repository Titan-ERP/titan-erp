...
---------
19.0.8.0.0
  - Fixed: replaced direct pg_dump subprocess call with odoo.service.db.dump_db
    so backups work correctly on Odoo SH where the direct call failed with exit
    code 1 (authentication, SSL, or pg_dump version mismatch with the server);
    Odoo's own backup function handles all connection details correctly
  - Changed: manifest.json is now generated by Odoo's own backup mechanism
    instead of a custom version, ensuring format compatibility
